...
class VL_Evaluator():
    ...
    def eval_batch(self, sample_list, image_dir):
        ...
        for sample in sample_list:
            ...
            if isinstance(sample["image"], list):
                for image in sample["image"]:
                    ...
                    binaryMap.append(np.load(path.replace("png", "npy").replace("VLM", "roadSoftMap")))
            else:
        ...
        print("fuck task_name", self.task_name)
        if self.task_name == "pointLabel13":
            cnt = 0
            while cnt < 10:
                ...
                for output, item in zip(outputs, sample_list):
                    ...
                    try:
                        pred_coords = extract_coordinates(generated_text)
                        binary_map_now = binaryMap[0]
                        dis = binary_map_now[int(pred_coords[0][0]), int(pred_coords[0][1])]
                        print("now_dis", dis)
                        if dis < 10:
                            flag = 0
                    except:
                ...
                cnt += 1
            if cnt == 10:
                print("Failed to generate valid coordinates after 10 attempts")
                return sample_list
        else:
            outputs = self.model.generate(prompts_text_and_vision, sampling_params=self.sampling_params, use_tqdm=False)
    ...
    def run(self, task_name="trance", eval_type="cot-sft", batch=16):
        ...
        assert task_name in ["trance", "trance-left", "trance-right", "clevr-math",
                             "TrajClassification", "pointLabel", "pointLabel13", "Trajectory",
        ...
        if eval_type == "cot-sft":
            ...
            elif task_name in ["TrajClassification", "pointLabel", "pointLabel13", "Trajectory"]:
        ...
        if task_name == "trance":
            ...
            self.image_dir = "/root/private_data/Reason-RFT/Reason-RFT-CoT-Dataset/images/test_images/"
            ...
            self.image_dir = "/root/private_data/Reason-RFT/preliminaryData"
            ...
            self.image_dir = "/root/private_data/Reason-RFT/VLM"
        ...
        for idx, sample in tqdm(enumerate(data), desc=f"{self.task_name}-{self.eval_type}", total=len(data)):
            ...
            if pred_times % 10 == 0:
                self.path_to_save = os.path.join(self.model_name_or_path, f"{args.city}-vision-r1-result")
        ...
        for sample in sample_list:
            ...
            if isinstance(sample["image"], list):
                for image in sample["image"]:
                    ...
                    binaryMap.append(np.load(os.path.join(image_dir, image).replace("png", "npy").replace("VLM", "roadSoftMap")))
            else:
                images = [Image.open(os.path.join(image_dir, sample["image"])) if isinstance(sample["image"], str) else sample["image"]]
                binaryMap.append(np.load(os.path.join(image_dir, sample["image"]).replace("png", "npy").replace("VLM", "roadSoftMap")))
