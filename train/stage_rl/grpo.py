...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["mse", "format", "reason", "tuple", "map"],
...
def main(script_args, training_args, model_args):
    ...
    assert script_args.task_name in ['trajectory', 'trance', 'trance-only-full', 'trance-full-caption', 'trance-penalty', 'clevr-math', 'geomath'], f"Task ({script_args.task_name}) is not supported."
    ...
    elif script_args.task_name == "trajectory":
        SYSTEM_PROMPT = GENERAL_SYSTEM_PROMPT
        QUESTION_PROMPT = TRAJECTORY_QUESTION_PROMPT
        reward_funcs_registry = {
            "mse": mse_reward,
            "format": format_reward,
            "reason": reasoning_steps_reward,
