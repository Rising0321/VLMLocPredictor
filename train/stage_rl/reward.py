...
def mse_reward(completions, answer, **kwargs):
    """Reward function that calculates the mean squared error between the predicted and ground truth."""
    ...
    for content, sol in zip(contents, answer):
        ...
        try:
            ...
            teacher_answer = sol_match.group(1).strip() if sol_match else sol.strip()
            pattern2 = r'the 13th point\((.*?),(.*?)\),\((.*?),(.*?)\)'
            match = re.search(pattern2, student_answer)
            if match:
                student_answer = match.group(1), match.group(2)
            pattern3 = r"\((.*?),(.*?)\),\((.*?),(.*?)\)"
            match2 = re.search(pattern3, teacher_answer)
            if match2:
                teacher_answer = match2.group(1), match2.group(2)
            # teacher_answer = int(sol[0]), int(sol[1])
            teacher_answer = int(teacher_answer[0]), int(teacher_answer[1])
            student_answer = int(student_answer[0]), int(student_answer[1])

            mae = math.sqrt((teacher_answer[0]-student_answer[0])**2+(teacher_answer[1]-student_answer[1])**2)
            if mae > 400:
                reward = 0
            else:
                reward = 1 - mae / 400
        ...
        if True:
            ...
            try:
                with open(log_path, "a") as f:
                    ...
                    f.write(f"Teacher Answer: {teacher_answer}\n")
                    f.write(f"Student Answer: {student_answer}\n")
            except:
                pass
    return rewards

def map_reward(completions, answer, binary_maps, **kwargs):
    """Reward function that calculates the mean squared error between the predicted and ground truth."""
    contents = [completion[0]["content"] for completion in completions]
    rewards = []
    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
    for content, sol, binary_map in zip(contents, answer, binary_maps):
        reward = 0.0
        # Try string matching
        teacher_answer = None
        student_answer = None
        try:
            ...
            if match:
                student_answer = match.group(1), match.group(2)
            student_answer = int(student_answer[0]), int(student_answer[1])
            reward = binary_map[student_answer[0], student_answer[1]]
        except Exception:
            pass  # Keep reward as 0.0 if both methods fail         
        ...
        if True:
            ...
            try:
                with open(log_path, "a") as f:
                    f.write(f"------------- {current_time} Map reward: {reward} -------------\n")
                    f.write(f"Content: {content}\n")
            except:
                pass
    return rewards
...
def tuple_reward(completions, **kwargs):
    """Reward function that checks if the completion has a specific format."""
